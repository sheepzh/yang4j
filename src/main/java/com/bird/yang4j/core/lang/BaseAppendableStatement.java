...
import com.bird.yang4j.core.lang.statement.Default;
...
public abstract class BaseAppendableStatement extends AbstractStatement implements ChildrenAppendable {
     ...
     */
    protected final void assertDefault(com.bird.yang4j.core.lang.statement.Type type, Default defaultVal) {
        if (defaultVal != null) {
            BuiltInType<?> builtInType = type.getArgumentJava();
            if (builtInType == BuiltInType.EMPTY) {
                throw new SyntaxException("An 'empty' type has no default value!");
            }
            try {
    ...
    public <T extends Statement> void append(T s) throws SyntaxException, NullPointerException {
        ...
        // Add the typedef the current context
        if (type.equals(Typedef.class)) {
            context.addDerivedType((Typedef) s);
        }
    }
                        ...
                        if (fc instanceof ParameterizedType) {
                            ...
                            if (types != null && types.length > 0) {
                                ...
                                if (type instanceof Class) {
                                    // 'type' is maybe one instance of ParameterizedTypeImpl
